...
//---------------------------------------------------------------------------
	...
	string COCKPIT_NAME = "Fighter Cockpit [Hydrogen]";
	int iceSurfaceNum = 1;
	int hydrogenSurfaceNum = 2;
...
void step5(){
	...
	if(iceSurface != null){
		...
		using(var frame = iceSurface.DrawFrame()){
			...
			MySprite iceTxt = MySprite.CreateText(s, "Debug", new Color(1f,1f,0f),0.45f,TextAlignment.CENTER);
			iceTxt.Position = new Vector2(50f);
			frame.Add(iceTxt);
		}
	}
	if(hydrogenSurface != null){
		...
		using(var frame = hydrogenSurface.DrawFrame()){
			...
			MySprite hydrogenTxt = MySprite.CreateText(h, "Debug", new Color(1f,1f,0f),0.45f,TextAlignment.CENTER);
			hydrogenTxt.Position = new Vector2(35f);
