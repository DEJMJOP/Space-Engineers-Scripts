...
void step2(){
	...
	Echo(BATTERIES.ElementAt(0).DetailedInfo);
}
void step3(){}
void step4(){
	IMyTextSurfaceProvider blk = GridTerminalSystem.GetBlockWithName(SEAT_NAME) as IMyTextSurfaceProvider;
...
void step5(){
	foreach(IMyTerminalBlock panel in PANELS){
		...
		for(int i = 0 ; i < BATTERY_POWER.Count ; i++){
			WriteToPanel((BATTERY_POWER.ElementAt(i)).ToString("0.##") + " MWh", panel, true);
			WriteToPanelLn(" | " + ((BATTERY_POWER.ElementAt(i)/((IMyBatteryBlock)BATTERIES.ElementAt(i)).MaxStoredPower)*100).ToString("0.##") + "%", panel, true);
			
			if( ((IMyBatteryBlock)BATTERIES.ElementAt(i)).IsCharging) WriteToPanel("Charging",panel,true);
			else WriteToPanel("Discharging",panel,true);
			
			String[] info = BATTERIES.ElementAt(i).DetailedInfo.Split(':');
			
			
			WriteToPanelLn(": " + info[info.Length-1], panel, true);
